(function (globals) {
  Ember.assert(
    "i18n-plurals must be included after i18n.",
    globals.Ember.I18n != null
  );
  ...
  defineLanguageRule(["be", "bs", "hr", "ru", "sr", "sh", "uk"], function (n) {
    ...
    if (
      isAmong(mod10, [0, 5, 6, 7, 8, 9]) ||
      isAmong(mod100, [11, 12, 13, 14])
    ) {
      return Many;
    }
  ...
  defineLanguageRule(["pl"], function (n) {
    ...
    if (
      isAmong(mod10, [0, 1, 5, 6, 7, 8, 9]) ||
      isAmong(mod100, [12, 13, 14])
    ) {
      return Many;
    }
  ...
  function pluralForm(count, language) {
    if (count == null) {
      throw new Error("Ember.I18n.pluralForm requires a count");
    }
    language = language || Ember.I18n.locale;
    if (language == null) {
      throw new Error("Ember.I18n.pluralForm requires a language");
    }
    language = language.replace(/^(\w\w\w?)-?.*/, "$1");
    if (Data[language] == null) {
      throw new Error("No pluralization information for " + language);
    }
